...
public class BatchExecutor {
  ...
  public <T> void execute(
      List<T> dataList, Consumer<T> consumer, int maxThreadCount, int chunkSize) {
    AtomicBoolean hasError = new AtomicBoolean(false);
    int total = dataList.size();
    try {
      MultiThreadedProcessing multiThreadedProcessing =
          new MultiThreadedProcessing(maxThreadCount, chunkSize);
      multiThreadedProcessing.execute(
          dataList,
          (index, data) -> {
            try {
